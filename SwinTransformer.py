...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, prev_v1, prev_k1, prev_q1):
        """ Forward function.
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.depth = depth
        self.use_checkpoint = use_checkpoint
        self.use_shared_qk = use_shared_qk
...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        self.layers_up_decoder2 = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer_up = BasicLayer_up(
                dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                input_resolution=(patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
            ...
            self.layers_up_decoder2.append(layer_up)
    ...
    def forward_features(self, x):
        ...
        return x, x_downsample, v_values_1, k_values_1, q_values_1

    # Dencoder1 
    def forward_up_features(self, x, x_downsample, v_values_1, k_values_1, q_values_1):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = x.permute(0, 4, 1, 2, 3)
                x = layer_up(x, v_values_1[3 - inx], k_values_1[3 - inx], q_values_1[3 - inx])
    ...
    def forward_up_features_decoder2(self, x, x_downsample, v_values_1, k_values_1, q_values_1):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x)
            else:
    ...
    def forward(self, x):
        ...
        x = self.forward_up_features(x, x_downsample, v_values_1, k_values_1, q_values_1, v_values_2, k_values_2,
                                     q_values_2)
        x = self.up_x4(x)
        x_decoder2 = self.forward_up_features_decoder2(x_downsample, q_values_1, k_values_1)
        x_decoder2 = self.up_x4(x_decoder2)
