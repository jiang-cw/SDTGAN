...
from SwinTransformer import SwinTransformerSys3D
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc=1, output_nc=1, num_downs=5 , ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=True):
...
def build_netG(opt):
    if opt.netG == 'resnet':
        ...
        generator = UnetGenerator(ngf=opt.ngf, use_dropout=True, norm_layer=nn.BatchNorm2d)
    elif opt.netG == 'STDGAN':
        generator = STDGAN(ngf=opt.ngf,num_classes= 1, embed_dim= 96, win_size= 7)
    else:
